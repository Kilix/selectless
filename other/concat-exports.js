...
const componentExports = htmlTagNames
  ...
  .map(tag => {
    ...
    return `export var ${tagName} = selectless['${capitalName}'];`
  }).join`\n`
...
const selectlessEsFile = resolve(__dirname, '..', 'dist', 'selectless.es.js')

appendToFile(selectlessEsFile, introText + componentExports)
